...
describe('secure EPUB conversion', () => {
  ...
  it('builds synchronized Markdown and visual EPUB sources for multimodal notebooks', () => {
    ...
    const beforeDiagram = llmBook.indexOf('Before the diagram.')
    const placedFigure = llmBook.indexOf('FIG-0001 - Flow chart', beforeDiagram)
    expect(beforeDiagram).toBeLessThan(placedFigure)
    expect(placedFigure).toBeLessThan(llmBook.indexOf('After the diagram.'))
