...
describe('passive searchable PDF builder', () => {
  it('combines page images with Unicode text without source objects', async () => {
    ...
    expect(document.getTitle()).toBe('Searchable test')
    ...
    try {
      const searchableDocument = await loadingTask.promise
      const firstPage = await searchableDocument.getPage(1)
      const firstPageText = await firstPage.getTextContent({ disableNormalization: false })
      const extractedText = firstPageText.items
      ...
      const extractedLongText = secondPageText.items
        .flatMap((item) => typeof item === 'object' && item !== null && 'str' in item && typeof item.str === 'string' ? [item.str] : [])
        .join('')
      expect(extractedLongText.length).toBe(longPageText.length)
      expect(extractedLongText).toBe(longPageText)
    } finally {
